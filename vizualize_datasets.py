...
def load_dataset(filepath):
    ...
    return np.array(data), np.array(labels), np.array(centroids), metadata
...
def create_info_box(ax, text, position='top_left', bg_color='#E8F4F8',
                   edge_color='#3498DB'):
...
def add_ellipses(ax, data, labels, centroids):
    ...
    for label in unique_labels:
        ...
        if len(cluster_points) > 1:
            ...
            ellipse = Ellipse(xy=centroids[label],
                             width=width, height=height,
                             angle=angle,
                             alpha=0.15,
                             color=plt.cm.tab20c(label / max(1, len(unique_labels)-1)),
                             linestyle='--', linewidth=1, edgecolor='gray')
            ax.add_patch(ellipse)


def plot_dataset_2d(data, labels, centroids, metadata, save_path):
    ...
    ax1 = axes[0]
    ax1.scatter(plot_data[:, 0], plot_data[:, 1],
                c=labels, cmap='tab20c', alpha=0.7, s=30,
                edgecolors='white', linewidth=0.5)

    ax1.scatter(plot_centroids[:, 0], plot_centroids[:, 1],
                c='#FF6B6B', marker='*', s=350, edgecolors='black',
    ...
    info_text = (f"Параметры датасета:\n"
    ...
    bars = ax2.bar(unique_labels, counts, color=colors, edgecolor='black',
    ...
    ax2.set_xticklabels([f'Кластер {i}' for i in unique_labels], rotation=45, ha='right')
    ...
    balance_stats = (f"Статистика баланса:\n"
...
def plot_dataset_3d(data, labels, centroids, metadata, save_path):
    ...
    ax1.scatter(data[:, 0], data[:, 1], data[:, 2],
                c=labels, cmap='tab20c', alpha=0.6, s=15,
                edgecolors='white', linewidth=0.3, depthshade=True)

    ax1.scatter(centroids[:, 0], centroids[:, 1], centroids[:, 2],
                c='#E74C3C', marker='*', s=300, edgecolors='black',
                linewidth=2, label='Центроиды', zorder=10)
    ...
    for pos, x_idx, y_idx, title in proj_config:
        ...
        ax.scatter(data[:, x_idx], data[:, y_idx],
                   c=labels, cmap='tab20c', alpha=0.5, s=10,
                   edgecolors='white', linewidth=0.2)
        ax.scatter(centroids[:, x_idx], centroids[:, y_idx],
                   c='#E74C3C', marker='*', s=200, edgecolors='black',
...
def plot_high_dim_stats(data, labels, metadata, save_path):
    ...
    ax1.bar(x, feature_means, yerr=feature_stds, capsize=5,
            alpha=0.8, color='#3498DB', edgecolor='#2C3E50', linewidth=1.5)

    ax1.set_xlabel('Индекс признака', fontweight='bold')
    ax1.set_ylabel('Среднее ± Стандартное отклонение', fontweight='bold')
    ax1.set_title(f'Распределение значений первых {n_features} признаков', pad=15)
    ax1.set_xticks(x)
    ax1.set_xticklabels([f'Признак {i+1}' for i in range(n_features)],
    ...
    ax2.hist(norms, bins=50, alpha=0.8, color='#27AE60',
             edgecolor='#145A32', linewidth=1.5)
    ...
    if data.shape[1] > 5:
        ...
        ax3.plot(range(1, len(cumsum) + 1), cumsum, 'b-o', linewidth=2.5,
                markersize=6, markerfacecolor='white', markeredgewidth=2,
                label='Накопленная дисперсия')
        ax3.axhline(y=0.95, color='#E74C3C', linestyle='--', linewidth=2,
                   alpha=0.7, label='95% дисперсии')
        ...
        ax3.text(0.5, 0.5, f'Размерность D={data.shape[1]} слишком мала для PCA\n(требуется D > 5)',
    ...
    if len(unique_labels) <= 12:
        wedges, texts, autotexts = ax4.pie(
            counts, labels=[f'Кластер {i}' for i in unique_labels],
        ...
        bars = ax4.bar(unique_labels, counts, color=colors, alpha=0.8,
...
def process_single_dataset(dataset_file, output_dir):
    """Обработка одного датасета."""
    try:
        data, labels, centroids, metadata = load_dataset(dataset_file)

        print(f"   Загружено: {len(data):,} точек, {data.shape[1]}D, "
...
def save_dataset_info(output_dir, dataset_name, dataset_file, data, labels, metadata):
