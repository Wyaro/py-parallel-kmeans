# K-Means Benchmark Suite

> Комплексная система для экспериментального исследования и сравнения вычислительной эффективности различных реализаций алгоритма K-means на CPU и GPU

---

## Содержание

- [Описание проекта](#описание-проекта)
- [Описание алгоритма K-means](#описание-алгоритма-k-means)
- [Основные возможности](#основные-возможности)
- [Быстрая установка](#быстрая-установка)
- [Быстрый старт](#быстрый-старт)
- [Эксперименты](#эксперименты)
- [Условия проведения экспериментов](#условия-проведения-экспериментов)
- [Реализации алгоритма](#реализации-алгоритма)
- [Метрики производительности](#метрики-производительности)
- [Анализ результатов](#анализ-результатов)
- [Структура проекта](#структура-проекта)
- [Расширение проекта](#расширение-проекта)
- [Тестирование](#тестирование)

---

## Описание проекта

**K-Means Benchmark Suite** — это исследовательская платформа для экспериментального исследования и сравнения вычислительной эффективности реализации алгоритма K-means на CPU и GPU, выявления узких мест производительности и закономерностей масштабирования, а также формирования практических рекомендаций по выбору архитектуры вычислений в зависимости от параметров задачи кластеризации.

### Цель работы

Целью данной работы является экспериментальное исследование и сравнение вычислительной эффективности реализации алгоритма «K-means» на CPU и GPU, выявление узких мест производительности и закономерностей масштабирования, а также формирование практических рекомендаций по выбору архитектуры вычислений в зависимости от параметров задачи кластеризации.

### Задачи исследования

- Разработать и реализовать однопоточную, многопоточную и GPU версии алгоритма K-means
- Измерить производительность на наборах данных различного масштаба
- Провести анализ масштабируемости по параметрам N, D, K
- Определить условия, при которых применение той или иной реализации является оптимальным
- Выявить узкие места производительности для каждой реализации

**Важное примечание**: Данное исследование посвящено именно анализу производительности вычислений, а не оценке качества кластеризации. Качество кластеризации проверяется лишь один раз для валидации корректности реализаций, но не является предметом основного исследования.

### Ключевые особенности

- **Множественные реализации**: CPU (NumPy, multiprocessing), GPU (CuPy/CUDA)
- **5 типов экспериментов** с автоматизированным запуском и сбором результатов
- **Генерация синтетических датасетов** с контролируемыми параметрами
- **Адаптивная остановка**: алгоритм останавливается при сходимости (максимум 100 итераций)
- **Потоковая запись результатов** в формате NDJSON по мере выполнения
- **Ограничение времени**: автоматический ранний выход при превышении лимита
- **Детальная статистика**: анализ времени выполнения по этапам и производственных метрик

---

## Описание алгоритма K-means

### Математическая формулировка

Алгоритм K-means позволяет решать следующую задачу кластеризации:

Дано множество точек **X = {x₁, x₂, …, xₙ}**, где **xᵢ ∈ ℝᴰ**, которые требуется разбить на **K** (заданное число) кластеров **C₁, …, Cₖ** таким образом, чтобы минимизировать функционал внутрикластерной суммы квадратов (WCSS, Within-Cluster Sum of Squares):

```
J = Σ(k=1 to K) Σ(xᵢ ∈ Cₖ) ||xᵢ - μₖ||²
```

где **μₖ** — центроид кластера **Cₖ**, вычисляемый как среднее арифметическое всех точек, принадлежащих кластеру:

```
μₖ = (1/|Cₖ|) Σ(xᵢ ∈ Cₖ) xᵢ
```

### Классический итерационный алгоритм (метод Ллойда)

Алгоритм состоит из следующих этапов:

1. **Инициализация центроидов**: происходит случайный выбор K начальных центроидов из набора данных
2. **Шаг присваивания**: назначение каждому объекту **xᵢ** ближайшего центроида **μₖ**, используя Евклидову метрику:
   ```
   cᵢ = argminₖ ||xᵢ - μₖ||²
   ```
3. **Шаг обновления**: пересчет центроидов на основе назначенных точек
4. **Критерий остановки**: шаги 2-3 повторяются до выполнения критерия сходимости или проведения заданного количества итераций

### Вычислительная сложность

Одна итерация алгоритма имеет сложность:

```
O(N × K × D)
```

где:
- **N** — количество точек
- **K** — число кластеров
- **D** — размерность пространства

---

## Основные возможности

### Генерация датасетов

Создание синтетических датасетов с различными параметрами для экспериментов:

```python
from generate_datasets import DatasetGenerator

generator = DatasetGenerator(base_seed=42)
generator.generate_all()  # Генерирует все типы датасетов
```

### Запуск экспериментов

Автоматизированные эксперименты по производительности:

```bash
# Все эксперименты (полный набор для комплексного анализа)
python -m kmeans.main --experiment all

# Конкретный эксперимент
python -m kmeans.main --experiment exp2_scaling_n

# С ограничением времени (30 минут)
python -m kmeans.main --experiment exp2_scaling_n --max-seconds 1800

# Только GPU реализации (пропустить CPU алгоритмы)
python -m kmeans.main --experiment exp2_scaling_n --gpu-only
```

### Анализ результатов

Статистический анализ времени выполнения:

```python
from analyze_timings import compute_stats_from_results

compute_stats_from_results("kmeans_timing_results.json", output_path="timings_report.txt")
```

---

## Быстрая установка

### Системные требования

- **Python**: 3.10 или выше
- **ОС**: Windows, Linux, macOS
- **GPU** (опционально): NVIDIA GPU с поддержкой CUDA для GPU-реализаций

### Установка зависимостей

#### 1. Клонирование репозитория

```bash
git clone <repository-url>
cd parallel
```

#### 2. Создание виртуального окружения

```bash
# Создание окружения
python -m venv .venv

# Активация окружения
# Windows:
.venv\Scripts\activate
# Linux/macOS:
source .venv/bin/activate
```

#### 3. Установка базовых зависимостей

```bash
pip install -r requirements.txt
```

#### 4. Установка GPU поддержки (опционально)

**ВАЖНО**: Установите только **ОДНУ** сборку CuPy, соответствующую вашей версии CUDA.

Сначала определите версию CUDA:

```bash
nvidia-smi
```

Затем установите соответствующую версию CuPy:

```bash
# Для CUDA 12.x
pip install cupy-cuda12x

# Или для CUDA 11.x
pip install cupy-cuda11x
```

**Примечание**: Если у вас установлено несколько версий CuPy, это может вызвать конфликты. Перед установкой новой версии удалите все существующие:

```bash
pip uninstall -y cupy cupy-cuda11x cupy-cuda12x cupy-cuda13x
pip install cupy-cuda12x  # или нужная вам версия
```

### Настройка в PyCharm

#### Способ 1: Автоматическая настройка

1. Откройте проект в PyCharm
2. При появлении уведомления "No Python interpreter configured" нажмите **"Configure Python Interpreter"**
3. Выберите **"New environment"**
4. Укажите Location: `.venv` (в корне проекта)
5. Выберите Base interpreter: Python 3.10 или выше
6. Убедитесь, что галочка **"Inherit global site-packages"** снята
7. Нажмите **"OK"**
8. PyCharm автоматически обнаружит `requirements.txt` и предложит установить зависимости

#### Способ 2: Ручная настройка

1. **File** → **Settings** (Ctrl+Alt+S)
2. **Project: parallel** → **Python Interpreter**
3. Нажмите на шестерёнку → **"Add..."**
4. Выберите **"New environment"**
5. Location: `.venv`, Base interpreter: Python 3.10+
6. Нажмите **"OK"**
7. В терминале PyCharm выполните:

```bash
pip install -r requirements.txt
```

### Проверка установки

```python
# Проверка базовых зависимостей
import numpy as np
import sklearn
print(f"NumPy версия: {np.__version__}")
print(f"SciKit-learn версия: {sklearn.__version__}")

# Проверка доступности GPU
from kmeans.core.gpu_numpy import gpu_available
print(f"GPU доступен: {gpu_available()}")
```

---

## Быстрый старт

### Шаг 1: Генерация датасетов

```bash
python main.py
```

Это создаст:
- Базовые датасеты для экспериментов
- Датасеты для масштабирования по N, D, K
- Валидационные датасеты
- Визуализации всех датасетов

### Шаг 2: Запуск экспериментов

```bash
# Все эксперименты (рекомендуется для полного анализа)
python -m kmeans.main --experiment all

# Или конкретный эксперимент
python -m kmeans.main --experiment exp1_baseline_single
python -m kmeans.main --experiment exp2_scaling_n
python -m kmeans.main --experiment exp3_scaling_d
python -m kmeans.main --experiment exp4_scaling_k
python -m kmeans.main --experiment exp5_gpu_profile
```

### Шаг 3: Анализ результатов

```bash
python analyze_timings.py
# Или программно:
python -c "from analyze_timings import compute_stats_from_results; compute_stats_from_results('kmeans_timing_results.json', output_path='timings_report.txt')"
```

---

## 🔬 Эксперименты

Проект включает **5 типов экспериментов** для всестороннего анализа производительности.

### Общие опции запуска

```bash
# Все эксперименты
python -m kmeans.main --experiment all

# С ограничением времени (30 минут)
python -m kmeans.main --experiment all --max-seconds 1800

# Только GPU реализации (пропустить CPU алгоритмы)
python -m kmeans.main --experiment all --gpu-only
```

### Эксперимент 1: Измерение эффективности однопоточных реализаций

**Цель**: Установить базовое время выполнения однопоточных реализаций

| Параметр | Значение |
|----------|----------|
| N | 100,000 |
| D | 50 |
| K | 8 |
| Реализации | `python_cpu_numpy` |
| Метрики | T_назн, T_обн, T_итер, T_общ |
| Повторы | 30 |
| Warmup | 3 |

**Запуск**:
```bash
python -m kmeans.main --experiment exp1_baseline_single
```

**Описание**: Этот эксперимент служит базовой линией для сравнения производительности. Результаты используются для расчёта метрик ускорения (Speedup) и параллельной эффективности (Efficiency) в других экспериментах.

---

### Эксперимент 2: Масштабирование по количеству точек (N)

**Цель**: Оценить зависимость времени выполнения от размера данных

| Параметр | Значения |
|----------|----------|
| N | {10³, 10⁵, 10⁶, 5×10⁶} |
| D | 50 (фиксировано) |
| K | 8 (фиксировано) |
| Реализации | CPU (single, multi-process), GPU (все варианты) |
| Повторы | 50, 20, 10, 5 соответственно |

**Запуск**:
```bash
python -m kmeans.main --experiment exp2_scaling_n
```

**Особенности**:
- Для больших датасетов (N ≥ 1,000,000) автоматически используются только оптимизированные GPU-реализации для предотвращения переполнения памяти
- Результаты включают метрики ускорения относительно baseline

---

### Эксперимент 3: Масштабирование по размерности (D)

**Цель**: Исследовать влияние размерности признакового пространства на производительность

| Параметр | Значения |
|----------|----------|
| D | {2, 10, 50, 100, 200} |
| N | 100,000 (фиксировано) |
| K | 8 (фиксировано) |
| Реализации | CPU (single, multi-process), GPU (все варианты) |
| Повторы | 20, 20, 10, 10, 10 |

**Запуск**:
```bash
python -m kmeans.main --experiment exp3_scaling_d
```

---

### Эксперимент 4: Масштабирование по количеству кластеров (K)

**Цель**: Исследовать влияние числа кластеров на время выполнения

| Параметр | Значения |
|----------|----------|
| K | {4, 8, 16, 32} |
| N | 100,000 (фиксировано) |
| D | 50 (фиксировано) |
| Реализации | CPU (single, multi-process), GPU (все варианты) |
| Повторы | 20, 20, 10, 10 |

**Запуск**:
```bash
python -m kmeans.main --experiment exp4_scaling_k
```

---

### Эксперимент 5: GPU профилирование

**Цель**: Детальный анализ GPU-реализации с использованием профилировщика

| Параметр | Значение |
|----------|----------|
| N | 1,000,000 |
| D | 50 |
| K | 8 |
| Инструменты | Nsight Compute |
| Метрики | Memory bandwidth utilization, Kernel occupancy, L1/L2 cache hit rate, Warp execution efficiency, T_transfer vs T_compute |
| Повторы | 10 |

**Запуск**:
```bash
python -m kmeans.main --experiment exp5_gpu_profile
```

**Примечание**: Этот эксперимент предназначен для детального анализа GPU-реализаций с использованием профилировщика Nsight Compute и включает метрики передачи данных (H2D, D2H) и их долю от общего времени выполнения.

---

## Условия проведения экспериментов

### Аппаратные условия

Все эксперименты должны проводиться на одной и той же аппаратной конфигурации для обеспечения корректности и воспроизводимости результатов.

**Спецификации оборудования**:

**CPU**:
- Модель: 12th Gen Intel(R) Core(TM) i5-12600KF
- Количество физических ядер: 10
- Количество логических ядер: 16
- Базовая/ускоренная частота: 3.7 ГГц / 4.9 ГГц
- Размер кэша L1/L2/L3:
  - P-core: 80 КБ/1280 КБ на ядро
  - E-core: 96 КБ/2 МБ на кластер
  - L3 (общий): 20 МБ

**GPU**:
- Модель: NVIDIA GeForce RTX 3050
- Compute Capability: 8.6
- Количество SM: 20
- Количество CUDA-ядер: 2560
- Объём видеопамяти: 8 ГБ
- Пропускная способность памяти: 224 ГБ/с
- Версия драйвера: 32.0.15.9144
- CUDA toolkit version: V13.1.80

**RAM**:
- Объём: 32 ГБ
- Тип: DDR4
- Частота (реальная): 1333 МГц
- Частота (эффективная): 2667 МГц

### Критерий остановки алгоритма

Для корректного сравнения производительности используется как единый критерий сходимости, так и фиксированное максимальное количество итераций кластеризации:

```
||μₖ^(t+1) - μₖ^t|| < ε, N_итер = 100
```

Ввиду того, что критерий сходимости может приводить к разному числу итераций в зависимости от особенностей реализации (порядок операций, точность вычислений с плавающей точкой), фиксированное число гарантирует, что каждая из реализаций выполняет одинаковый объём вычислений, что позволит корректно сравнивать производительность.

### Измерение времени

Для измерения вычислительной части (без ввода и вывода) для различных реализаций используются высокоточные таймеры, предоставляемые языком и архитектурой:

- **Python**: `time.perf_counter()`
- **C#**: `System.Diagnostics.Stopwatch`
- **C++**: `std::chrono::steady_clock`
- **CUDA**: `cudaEvent_t`

В рамках процедуры измерения происходит выполнение 3 итераций алгоритма без изменения времени (warm-up), после чего происходит основное измерение: заданное количество повторов полного цикла с фиксацией времени. Затем происходит статистическая обработка и вычисление целевых показателей.

### Описание используемых наборов данных

Для всесторонней оценки производительности используются синтетические наборы данных с систематическим варьированием параметров.

Генерация происходит за счет применения библиотеки Scikit-learn с фиксированным `random_state = 42` для последующей воспроизводимости.

**Параметры варьирования**:
- Количество точек: **N ∈ {10³, 10⁵, 10⁶, 5×10⁶}**
- Размерность: **D ∈ {2, 10, 50, 100, 200}**
- Количество кластеров: **K ∈ {4, 8, 16, 32}**

**Базовая конфигурация** (для сравнительных экспериментов):
- **N = 100,000** точек
- **D = 50** признаков
- **K = 8** кластеров

Эксперименты проводятся с варьированием одного параметра при фиксации остальных на базовых значениях.

---

## Реализации алгоритма

### CPU NumPy (Однопоточная)

Базовая реализация на NumPy для последовательных вычислений.

**Использование**:
```python
from kmeans.core.cpu_numpy import KMeansCPUNumpy

model = KMeansCPUNumpy(n_clusters=8, n_iters=100, tol=1e-6)
model.fit(X, initial_centroids)
```

**Особенности**:
- Прямолинейная реализация для максимальной ясности
- Использует векторные операции NumPy
- Подходит для небольших и средних датасетов
- Сложность одной итерации: O(N × K × D)
- Адаптивная остановка при сходимости (tol=1e-6) или максимум 100 итераций

---

### CPU Multiprocessing (Многопроцессная)

Параллельная реализация с использованием `multiprocessing.Pool` и разделяемой памяти.

**Использование**:
```python
from kmeans.core.cpu_multiprocessing import (
    KMeansCPUMultiprocessing,
    MultiprocessingConfig
)

config = MultiprocessingConfig(n_processes=4)
model = KMeansCPUMultiprocessing(n_clusters=8, mp=config)
model.fit(X, initial_centroids)
```

**Особенности**:
- **Разделяемая память** для данных (RawArray) — минимизация копирования
- **Параллелизация шага назначения** кластеров по чанкам объектов
- **Параллельная редукция** для обновления центроидов
- **Переиспользование пула процессов** между итерациями
- Оптимизировано для многоядерных систем
- Адаптивная остановка при сходимости (tol=1e-6) или максимум 100 итераций

---

### GPU CuPy (CUDA)

Несколько оптимизированных GPU-реализаций на CuPy.

#### Базовая версия (`KMeansGPUCuPy`)

**Использование**:
```python
from kmeans.core.gpu_numpy import KMeansGPUCuPy

model = KMeansGPUCuPy(n_clusters=8)
model.fit(X, initial_centroids)
```

**Особенности**:
- Прямое вычисление расстояний через broadcasting
- Использование `einsum` для оптимизации
- Перенос данных на GPU один раз
- Адаптивная остановка при сходимости (tol=1e-6) или максимум 100 итераций

#### Оптимизированная версия (`KMeansGPUCuPyFast`)

**Использование**:
```python
from kmeans.core.gpu_numpy import KMeansGPUCuPyFast

model = KMeansGPUCuPyFast(n_clusters=8, dtype=cp.float32)
model.fit(X, initial_centroids)
```

**Особенности**:
- Оптимизированное вычисление расстояний без материализации полного тензора `diff`
- Использование формулы: `||x - c||² = ||x||² + ||c||² - 2x·c`
- Эффективная редукция через `bincount` для обновления центроидов
- Минимизация использования памяти GPU
- По умолчанию использует `float32` для лучшей производительности
- Автоматический возврат результатов на CPU

#### Версия с raw CUDA kernel (`KMeansGPUCuPyRaw`)

**Использование**:
```python
from kmeans.core.gpu_numpy import KMeansGPUCuPyRaw

model = KMeansGPUCuPyRaw(n_clusters=8, dtype=cp.float32)
model.fit(X, initial_centroids)
```

**Особенности**:
- Кастомный raw CUDA kernel для шага назначения кластеров
- Максимальная производительность для больших датасетов
- Оптимизированная работа с памятью GPU
- По умолчанию использует `float32`

---

## Метрики производительности

Для объективной оценки вычислительной эффективности используются следующие количественные метрики:

### Временные метрики

| Метрика | Альтернативное название | Описание |
|---------|------------------------|----------|
| `T_assign` | `T_назн` | Время выполнения шага назначения: затраченное на вычисление расстояний от всех точек до всех центроидов и определение ближайшего центроида (на одну итерацию, мс) |
| `T_update` | `T_обн` | Время выполнения шага обновления центроидов: пересчет центроидов, характеризующий эффективность операции редукции (на одну итерацию, мс) |
| `T_iter` | `T_итер` | Полное время одной итерации алгоритма: `T_итер = T_назн + T_обн` (мс) |
| `T_total` | `T_общ` | Общее время выполнения до сходимости: `T_общ = Σ(t=1 to N_итер) T_итер^(t)` (мс) |

### Метрики параллелизма

| Метрика | Формула | Описание |
|---------|---------|----------|
| **Speedup (S)** | `S = T_опр / T_мпр` | Ускорение: где `T_опр` — время выполнения однопоточной реализации, а `T_мпр` — многопоточной соответственно |
| **Parallel Efficiency (E)** | `E = S / p` | Параллельная эффективность: где `p` — число потоков (CPU) или количество блоков (GPU) |

**Примечание**: Для GPU реализаций `p = 1`, поэтому вычисляется только Speedup без Efficiency.

### Метрики пропускной способности

| Метрика | Формула | Описание |
|---------|---------|----------|
| **Throughput (ПС)** | `ПС = (N × K × D × N_итер) / T_общ` | Пропускная способность (операций в секунду) |

### Метрики передачи данных (GPU)

| Метрика | Формула | Описание |
|---------|---------|----------|
| `T_transfer` | `T_пер = T_H2D + T_D2H` | Время передачи данных: общее время передачи данных между CPU и GPU |
| `R_transfer` | `R_пер = (T_пер / T_общ) × 100%` | Доля времени на передачу данных: процент времени передачи данных от общего времени выполнения |

**Где**:
- `T_H2D`: время передачи данных с CPU на GPU (Host-to-Device)
- `T_D2H`: время передачи данных с GPU на CPU (Device-to-Host)

---

## Анализ результатов

### Формат результатов

Результаты сохраняются в формате **NDJSON** (каждая строка — отдельный JSON-объект) в файл `kmeans_timing_results.json`:

```json
{
  "experiment": "exp2_scaling_n",
  "implementation": "python_cpu_numpy",
  "threads": null,
  "dataset": {
    "N": 100000,
    "D": 50,
    "K": 8,
    "purpose": "scaling_by_N"
  },
  "timing": {
    "T_fit_avg": 20.226,
    "T_fit_med": 20.145,
    "T_assign_total_avg": 18.056,
    "T_update_total_avg": 2.165,
    "T_iter_total_avg": 20.221,
    "speedup": 1.0,
    "efficiency": 1.0,
    "throughput_ops_avg": 197500000.0,
    "T_transfer_avg": null,
    "T_transfer_ratio_avg": null,
    "runs": [...]
  }
}
```

### Статистический анализ

Модуль `analyze_timings.py` предоставляет инструменты для анализа результатов:

```python
from analyze_timings import compute_stats_from_results, TimingResultsAnalyzer

# Простой анализ с сохранением в файл
compute_stats_from_results(
    "kmeans_timing_results.json",
    n_iters=100,
    output_path="timings_report.txt"
)

# Программный анализ
analyzer = TimingResultsAnalyzer("kmeans_timing_results.json", n_iters=100)
analyzer.analyze(output_path="timings_report.txt")
```

**Вывод включает**:
- Средние и медианные значения времени назначения кластеров (на одну итерацию)
- Средние и медианные значения времени обновления центроидов (на одну итерацию)
- Средние и медианные значения времени одной итерации
- Средние и медианные значения общего времени выполнения алгоритма

---

## Структура проекта

```
parallel/
├── main.py                      # Основной скрипт подготовки данных
├── generate_datasets.py          # Генератор синтетических датасетов
├── vizualize_datasets.py        # Визуализация датасетов
├── analyze_timings.py           # Анализ результатов экспериментов
│
├── datasets/                     # Сгенерированные датасеты
│   ├── base/                    # Базовые датасеты
│   ├── scaling_N/               # Масштабирование по N
│   ├── scaling_D/               # Масштабирование по D
│   ├── scaling_K/               # Масштабирование по K
│   ├── validation/              # Валидационные датасеты
│   └── datasets_summary.json    # Сводная информация
│
├── kmeans/                      # Основной пакет
│   ├── main.py                  # CLI для запуска экспериментов
│   │
│   ├── core/                    # Реализации алгоритма
│   │   ├── base.py              # Базовый абстрактный класс
│   │   ├── cpu_numpy.py         # Однопоточная CPU реализация
│   │   ├── cpu_multiprocessing.py  # Многопроцессная CPU реализация
│   │   └── gpu_numpy.py         # GPU реализация (CuPy/CUDA)
│   │
│   ├── data/                    # Управление данными
│   │   ├── dataset.py           # Класс Dataset
│   │   ├── registry.py          # Реестр датасетов
│   │   └── validation.py        # Валидация датасетов
│   │
│   ├── experiments/             # Эксперименты
│   │   ├── config.py            # Конфигурация экспериментов
│   │   ├── runner.py            # Запуск экспериментов
│   │   └── suite.py             # Оркестрация экспериментов
│   │
│   ├── metrics/                 # Метрики производительности
│   │   ├── metrics.py           # Ускорение, эффективность, пропускная способность
│   │   └── timers.py            # Высокоточные таймеры
│   │
│   └── utils/                   # Вспомогательные утилиты
│       └── logging.py           # Настройка логирования
│
├── kmeans_timing_results.json  # Результаты экспериментов (NDJSON)
├── visualizations/              # Визуализации датасетов
│
├── tests/                       # Тесты проекта
│   ├── conftest.py             # Фикстуры с тестовыми данными
│   ├── test_core/               # Тесты реализаций алгоритма
│   │   ├── test_cpu_numpy.py   # Unit-тесты CPU реализации
│   │   └── test_consistency.py # Тесты согласованности реализаций
│   └── test_metrics/            # Тесты метрик и таймеров
│
├── requirements.txt             # Зависимости проекта
└── README.md                    # Документация проекта
```

---

## Расширение проекта

### Добавление новой реализации алгоритма

1. Создайте класс, наследующий `KMeansBase`:

```python
from kmeans.core.base import KMeansBase
import numpy as np

class MyKMeans(KMeansBase):
    """
    Описание вашей реализации.
    """
    
    def assign_clusters(
        self, X: np.ndarray, centroids: np.ndarray
    ) -> np.ndarray:
        """
        Шаг назначения точек кластерам.
        
        Args:
            X: Массив данных (N, D)
            centroids: Текущие центроиды (K, D)
            
        Returns:
            Метки кластеров (N,)
        """
        # Ваша реализация шага назначения
        pass
    
    def update_centroids(
        self, X: np.ndarray, labels: np.ndarray
    ) -> np.ndarray:
        """
        Шаг обновления центроидов.
        
        Args:
            X: Массив данных (N, D)
            labels: Метки кластеров (N,)
            
        Returns:
            Новые центроиды (K, D)
        """
        # Ваша реализация шага обновления
        pass
```

2. Добавьте экспорт в `kmeans/core/__init__.py`:

```python
from .my_kmeans import MyKMeans

__all__ = [..., "MyKMeans"]
```

3. Используйте в экспериментах через фабрику:

```python
def _my_implementation_factory(**kw):
    return MyKMeans(n_clusters=kw['n_clusters'], logger=kw.get('logger'))
```

### Добавление нового эксперимента

1. Добавьте идентификатор в `kmeans/experiments/config.py`:

```python
class ExperimentId(str, Enum):
    # ... существующие эксперименты
    MY_EXPERIMENT = "exp6_my_experiment"
```

2. Добавьте конфигурацию:

```python
EXPERIMENTS: Dict[ExperimentId, ExperimentConfig] = {
    # ... существующие конфигурации
    ExperimentId.MY_EXPERIMENT: ExperimentConfig(
        id=ExperimentId.MY_EXPERIMENT,
        description="Описание эксперимента",
        implementations=["my_impl"],
        params={
            "N": 100000,
            "D": 50,
            "K": 8,
            "repeats": 20,
        },
    ),
}
```

3. Реализуйте метод в `ExperimentSuite`:

```python
def run_exp6_my_experiment(self) -> List[dict]:
    """
    Описание эксперимента.
    
    Returns:
        Список результатов экспериментов
    """
    cfg = EXPERIMENTS[ExperimentId.MY_EXPERIMENT]
    # Ваша логика эксперимента
    results: List[dict] = []
    # ...
    return results
```

4. Добавьте обработку в `kmeans/main.py`:

```python
elif args.experiment == ExperimentId.MY_EXPERIMENT.value:
    suite = make_suite_single()
    results = suite.run_exp6_my_experiment()
```

---

## Тестирование

Проект включает комплексный набор тестов для обеспечения корректности работы всех реализаций алгоритма.

### Запуск тестов

```bash
# Все тесты
pytest tests/

# Только тесты согласованности (критично для проверки корректности)
pytest tests/test_core/test_consistency.py -v

# С покрытием кода
pytest tests/ --cov=kmeans --cov-report=html

# Только быстрые тесты (без GPU)
pytest tests/ -m "not gpu"
```

### Структура тестов

- **Тесты согласованности** (`test_consistency.py`) — проверяют, что все реализации дают одинаковые результаты. Критически важно для корректности бенчмарков.
- **Unit-тесты реализаций** (`test_cpu_numpy.py`) — проверяют корректность работы отдельных компонентов алгоритма.
- **Тесты метрик** (`test_metrics/`) — проверяют корректность вычисления метрик производительности.

### Что тестируется

1. **Согласованность реализаций** — CPU NumPy, CPU Multiprocessing, GPU CuPy и GPU CuPy Fast должны давать одинаковые центроиды на одинаковых данных.
2. **Корректность алгоритма** — проверка шагов `assign_clusters` и `update_centroids`, обработка пустых кластеров.
3. **Метрики производительности** — корректность вычисления speedup, efficiency, throughput.

---

## Решение проблем

### Проблема: CuPy не устанавливается

**Решение**:
- CuPy требует CUDA Toolkit. Установите его с официального сайта NVIDIA
- Используйте правильную версию: `cupy-cuda12x` для CUDA 12.x, `cupy-cuda11x` для CUDA 11.x
- GPU поддержка опциональна — проект работает и без неё

### Проблема: OutOfMemoryError на GPU

**Решение**:
- Для больших датасетов (N ≥ 1,000,000) система автоматически использует только оптимизированные GPU-реализации
- Если проблема сохраняется, уменьшите размер датасета или используйте CPU-реализации

### Проблема: PyCharm не видит виртуальное окружение

**Решение**:
1. **File** → **Settings** → **Project: parallel** → **Python Interpreter**
2. Нажмите на шестерёнку → **"Add..."**
3. Выберите **"Existing environment"**
4. Укажите путь к `python.exe` в `.venv\Scripts\python.exe` (Windows) или `.venv/bin/python` (Linux/macOS)

### Проблема: Ошибки при установке зависимостей

**Решение**:
- Убедитесь, что используете Python 3.10 или выше
- Обновите pip: `python -m pip install --upgrade pip`
- Установите зависимости по одной для диагностики:

```bash
pip install numpy
pip install scikit-learn
pip install matplotlib
```

---

## Дополнительные ресурсы

- [Документация NumPy](https://numpy.org/doc/)
- [Документация CuPy](https://docs.cupy.dev/)
- [Алгоритм K-means (Wikipedia)](https://en.wikipedia.org/wiki/K-means_clustering)
- [Документация pytest](https://docs.pytest.org/)

---

## Авторы

О-21-ИАС-аид-С
Еремин И. А.
Проект разработан для исследования производительности алгоритма K-means на различных архитектурах вычислений.

---

**Версия**: 1.0.1  
**Последнее обновление**: Декабрь 2024
