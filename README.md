...
**K-Means Benchmark Suite** — это исследовательская платформа для количественной оценки вычислительной эффективности различных реализаций алгоритма K-means и выявления условий, при которых применение той или иной архитектуры вычислений является наиболее целесообразным.

### Цели исследования

- **Сравнительный анализ** производительности однопоточных, многопроцессных и GPU реализаций
...
**Цель**: Детальный анализ GPU-реализаций для профилирования производительности
...
**Примечание**: Этот эксперимент предназначен для детального анализа GPU-реализаций и включает метрики передачи данных (H2D, D2H) и их долю от общего времени выполнения.
...
Проект вычисляет следующие метрики производительности:

### Временные метрики

| Метрика | Описание |
|---------|----------|
| `T_assign` | Время назначения кластеров на одну итерацию (мс) |
| `T_update` | Время обновления центроидов на одну итерацию (мс) |
| `T_iter` | Время одной итерации алгоритма (мс) |
| `T_total` | Общее время выполнения алгоритма (мс) |

### Метрики параллелизма

| Метрика | Формула | Описание |
|---------|---------|----------|
| **Speedup (S)** | `S = T_serial / T_parallel` | Ускорение параллельной реализации относительно последовательной |
| **Parallel Efficiency (E)** | `E = S / p` | Параллельная эффективность, где `p` — число потоков/процессов |
...
| Метрика | Формула | Описание |
|---------|---------|----------|
| **Throughput (PS)** | `PS = (N × K × D × n_iters) / T_total` | Пропускная способность (операций в секунду) |

### Метрики передачи данных (GPU)

| Метрика | Формула | Описание |
|---------|---------|----------|
| `T_transfer` | `T_H2D + T_D2H` | Общее время передачи данных между CPU и GPU |
| `R_transfer` | `(T_transfer / T_total) × 100%` | Доля времени передачи данных от общего времени выполнения |
