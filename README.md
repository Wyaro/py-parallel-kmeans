...
pip install cupy-cuda13x  # для CUDA 13.x
# или cupy-cuda12x для CUDA 12.x
# или cupy-cuda11x для CUDA 11.x
```
...
5. Для GPU поддержки установите CuPy: `pip install cupy-cuda13x` (для CUDA 13.x) или `cupy-cuda12x` (для CUDA 12.x) или `cupy-cuda11x` (для CUDA 11.x)
...
# Strong scaling (масштабирование по числу процессов)
...
Эта команда запустит все 6 типов экспериментов с соответствующими параметрами и реализациями. Результаты будут сохраняться в `kmeans_timing_results.json` по мере выполнения.
